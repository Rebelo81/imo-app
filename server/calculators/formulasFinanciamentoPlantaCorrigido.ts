...
export function calcularFinanciamentoPlanta(input: FinanciamentoPlantaInput): ResultadoFinanciamentoPlanta {
  console.log("Iniciando cálculo de financiamento na planta (v3 corrigido):", JSON.stringify(input, null, 2));
  ...
  // Mês 0 = Saldo liquido em branco (null ou undefined)
  // Mês 1 = Valor do imóvel - entrada - desconto
  // Mês 2+ = Resultado saldo mês anterior - pagamento total líquido mês anterior
  ...
  if (tipoParcelamento === 'automatico') {
    ...
    for (let mes = 1; mes <= prazoPagamento; mes++) {
      ...
      if (mes === 1) {
        ...
        console.log(`[FÓRMULA] Mês ${mes}: SaldoLiquido = ${valorImovel} - ${valorEntradaEfetivo} - ${valorDescontoEfetivo} = ${saldoLiquidoAtual}`);
      } else {
        ...
        if (parcelaAnterior) {
          ...
          // CORREÇÃO: Usar valorCorrigido em vez de valorBase  
          const pagamentoMesAnterior = parcelaAnterior.valorCorrigido;
          
          // Calcular o novo saldo líquido
          saldoLiquidoAtual = saldoLiquidoMesAnterior - pagamentoMesAnterior;
          
          console.log(`[FÓRMULA] Mês ${mes}: SaldoLiquido = ${saldoLiquidoMesAnterior} - ${pagamentoMesAnterior} = ${saldoLiquidoAtual}`);
        } else {
    ...
    // IMPORTANTE: Processar TODOS os meses sequencialmente, como no automático
    for (let mes = 1; mes <= prazoPagamento; mes++) {
      // Procurar se há uma parcela personalizada para este mês
      const parcelaPersonalizada = parcelasOrdenadas.find(p => p.mes === mes);
      
      // Valor e tipo da parcela (zero se não há pagamento neste mês)
      const valorBase = parcelaPersonalizada ? parcelaPersonalizada.valor : 0;
      const tipoPagamento = parcelaPersonalizada ? parcelaPersonalizada.tipo : 'Parcela';
      ...
      // Atualizar saldo devedor com correção (MESMA LÓGICA DO AUTOMÁTICO)
      const correcaoMensal = saldoDevedorAtual * (percentualCorrecao / 100);
      saldoDevedorAtual += correcaoMensal;
      
      // Calcular correção acumulada (MESMA LÓGICA DO AUTOMÁTICO)
      const correcaoAcumulada = mes === 1 ? 
                              percentualCorrecao : 
                              parcelas[mes - 1].correcaoAcumulada + percentualCorrecao;
      
      // Calcular valor corrigido da parcela (MESMA LÓGICA DO AUTOMÁTICO)
      const valorCorrigido = valorBase * (1 + (correcaoAcumulada / 100));
      
      // Atualizar saldo devedor após o pagamento (MESMA LÓGICA DO AUTOMÁTICO)
      saldoDevedorAtual -= valorCorrigido;
      
      // CÁLCULO DO SALDO LÍQUIDO USANDO A MESMA FÓRMULA DO AUTOMÁTICO:
      let saldoLiquidoAtual = 0;
      
      if (mes === 1) {
        // Mês 1: Valor do imóvel - entrada - desconto
        saldoLiquidoAtual = valorImovel - valorEntradaEfetivo - valorDescontoEfetivo;
        console.log(`[PERSONALIZADO] Mês ${mes}: SaldoLiquido = ${valorImovel} - ${valorEntradaEfetivo} - ${valorDescontoEfetivo} = ${saldoLiquidoAtual}`);
      } else {
        ...
        if (parcelaAnterior) {
          ...
          console.log(`[PERSONALIZADO] Mês ${mes}: SaldoLiquido = ${saldoLiquidoMesAnterior} - ${pagamentoMesAnterior} = ${saldoLiquidoAtual}`);
        } else {
          // Caso de contingência (não deveria acontecer)
          console.log(`[ALERTA] Mês ${mes}: Não encontrou mês anterior ${mesAnterior}, usando valor inicial`);
          saldoLiquidoAtual = valorImovel - valorEntradaEfetivo - valorDescontoEfetivo;
      ...
      // Adicionar parcela (SEMPRE, mesmo se valorBase = 0)
      parcelas.push({
        mes,
        data: formatarData(dataBase, mes),
        tipoPagamento,
